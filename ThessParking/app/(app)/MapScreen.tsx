  ...
  useEffect(() => {
    const updateUserLocation = async () => {
      ...
      markers.forEach(marker => {
        const distance = getDistance(latitude, longitude, marker.latitude, marker.longitude);
        if (distance <= 0.002) {  // ✅ 2 meters threshold (converted to km)
          isUserNear = true;
        }
      });
...
const handleReport = async (option: "notAvailable" | "notValid" | "other") => {
    if (!selectedMarker) {
  ...
  const handleAddMarker = async () => {
    try {
      const location = await Location.getCurrentPositionAsync({});
      const newMarker = {
        coordinates: { latitude: location.coords.latitude, longitude: location.coords.longitude }, // Example position
        notAvailableCount: 0,
        notValidCount: 0,
        status: "active",
        user: "testUser",
    ...
    <ActionSheetProvider>
      <GestureHandlerRootView style={styles.container}>
        <View style={StyleSheet.absoluteFill}>
          ...
          />
          <BottomDrawer isVisible={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} onClaim={handleClaimMarker} onReport={handleReport}/>
